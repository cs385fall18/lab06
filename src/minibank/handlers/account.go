...
func getSessionLookup() func(string) (string, bool) {
	...
	if models.CassandraEnabled {
		return func(session string) (string, bool) {
			...
			// models.CassandraSession.Query
			//var id gocql.UUID
			//var text string
			row := models.CassandraSession.Query("SELECT username FROM sessions Where session = ?", session).Iter()
			for row.Scan(&username) {
...
func getSessionWriter() func(uuid.UUID, string) {
	...
	if models.CassandraEnabled {
		return func(session uuid.UUID, username string) {
			...
			err := models.CassandraSession.Query(`INSERT INTO sessions (session, username, expiration) VALUES (?, ?, ?)`,
							session.String(),
							username,
							uint64(time.Now().UnixNano()/1000000) + sessionDuration).Exec();
			if err != nil {
				log.Fatal(err)
			}
		}
...
func getSessionListLookup() func(string) UserSessions {
	...
	if models.CassandraEnabled {
		return func(username string) UserSessions {
			...
			*/
			rows := models.CassandraSession.Query("SELECT sessions FROM sessions Where uername = ?", username).Iter()
                        for rows.Scan(&username) {
				sessionList = append(sessionList, username)
                          //      return username, true
                                //fmt.PrintLn(id, text)
                        }
