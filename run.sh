...
docker run -d --name minibank -e JWT_SECRET_KEY=minibank ENABLE_CASSANDRA=TRUE -p 80:8080 --network minibanknet gcr.io/`gcloud config get-value project`/minibank:latest
